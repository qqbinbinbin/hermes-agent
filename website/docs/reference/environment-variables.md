...
| `FUXI_CONTRACT_AUTH_MODE` | Optional contract auth mode. Set to `hmac` for internal host calls that sign the request body instead of sending a bearer JWT; the default mode uses JWT. |
| `FUXI_CONTRACT_HMAC_SECRET_ENV` | Environment variable name that holds the HMAC secret when `FUXI_CONTRACT_AUTH_MODE=hmac` (default: `HERMES_INGEST_HMAC_KEY`). |
| `FUXI_CONTRACT_JWT_ENDPOINT` | Internal executor-gateway endpoint used to mint a short-lived director JWT for contract calls when no bearer JWT is configured. |
| `DIRECTOR_JWT_JWKS_URL` | JWKS URL used as a fallback hint for the internal director JWT exchange endpoint when `FUXI_CONTRACT_JWT_ENDPOINT` is unset. |
| `FUXI_CONTRACT_GOAL_ID` | Optional goal identifier forwarded to the contract endpoint and to the director JWT exchange request. |
