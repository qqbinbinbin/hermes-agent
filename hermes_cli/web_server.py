...
def _ws_client_reason(ws: "WebSocket") -> Optional[str]:
    ...
    if not client_host:
        # Loopback-bound dashboards must fail closed on an unidentified peer.
        # Explicit public binds and gated-mode upgrades can legitimately lose
        # the peer tuple behind a proxy, so those paths are allowed to
        # continue and rely on auth + Host/Origin checks instead.
        if _is_public_bind() or getattr(app.state, "auth_required", False):
            return None
        return f"missing_or_empty_peer bound={bound_host or '?'}"
    if client_host in _LOOPBACK_HOSTS:
        return None
    if _is_public_bind() and os.environ.get("HERMES_DASHBOARD_ALLOW_REMOTE_CHAT_WS") == "1":
        return None
    if _is_public_bind():
        return f"remote_chat_ws_disabled peer={client_host}"
...
def _ws_client_is_allowed(ws: "WebSocket") -> bool:
    ...
    Allows loopback always. Remote dashboard Chat WebSockets require both
    public bind and the explicit FUXI deployment flag; session token auth alone
    is not sufficient for managed remote access.
    ...
    Gated mode still requires the explicit remote-Chat opt-in for
    non-loopback peers. Uvicorn's ``proxy_headers=True`` (enabled when the
    OAuth gate is active so cookies can pick up ``X-Forwarded-Proto``)
    rewrites ``ws.client.host`` to the X-Forwarded-For value, which is the
    real internet client IP. The OAuth gate + single-use ``?ticket=`` is the
    auth at that point; the Host/Origin guard in
    :func:`_ws_host_origin_is_allowed` is what blocks DNS-rebinding here, and
    the peer-IP gate still enforces the FUXI remote-Chat flag for non-loopback
    peers.
    """
