...
class TestWsRequestIsAllowedGated:
    ...
    def test_non_loopback_peer_allowed_in_insecure_public_mode(
        self, insecure_public_app, monkeypatch
    ):
        """Public bind plus the explicit remote-WS flag allows LAN peers.
        ...
        with 403. A public bind alone is not sufficient: FUXI requires the
        separately governed remote-chat flag.
        """
        monkeypatch.setenv("HERMES_DASHBOARD_ALLOW_REMOTE_CHAT_WS", "1")
    ...
    def test_peer_allowed_on_explicit_non_loopback_bind(
        self, insecure_explicit_host_app, monkeypatch
    ):
        """The explicit remote-WS flag also covers a fixed LAN/tailnet bind.
